...
if (is_null($input) || !property_exists($input, 'playerid')) {
    Http::ReturnError(400, array('message' => 'Passenger details are empty, or player id not provided.'));
} else {
    try {
        // Create Db object
        ...
        if ($db->execute() === 0) {
            ...
            // Create Db object
            $db = new Db('UPDATE `passenger` SET playerid = :playerid, datemodified = :datemodified WHERE id = :id');
            ...
            Http::ReturnSuccess(array('message' => 'Passenger Player ID set.', 'id' => $input->id));
        }
